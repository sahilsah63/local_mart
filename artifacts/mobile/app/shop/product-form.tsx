...
export default function ProductForm() {
  ...
  const params = useLocalSearchParams<{ shopId?: string; productId?: string }>();
  ...
  const onSubmit = async () => {
    ...
    if (!price.trim() || isNaN(Number(price))) return Alert.alert("Required", "Valid price is required.");
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: 16, paddingTop: topPad }}>
      <Text style={{ fontSize: 22, fontWeight: "700", color: colors.foreground, marginBottom: 16 }}>
        {isEdit ? "Edit Product" : "Add Product"}
      </Text>

      <Field label="Product name *" colors={colors}>
        <TextInput value={name} onChangeText={setName} placeholder="e.g. Type-C Charger 25W"
      ...
      <View style={[styles.row, { borderColor: colors.border, backgroundColor: colors.card }]}>
        <View style={{ flex: 1 }}>
          <Text style={{ fontWeight: "600", color: colors.foreground }}>Active (visible to customers)</Text>
          <Text style={{ fontSize: 12, color: colors.mutedForeground }}>Turn off to hide without deleting</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderRadius: 10, padding: 12, marginBottom: 20 },
