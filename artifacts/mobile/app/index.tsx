...
//   const [activeCategory, setActiveCategory] = useState("all");
  ...
  // Use saved user coords if present, else get device GPS, else fallback to Lucknow
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (status !== "granted") {
          setCoords({ lat: 26.8467, lng: 80.9462 }); // Lucknow fallback
          return;
        }
  ...
  const shopParams: any = { limit: 10 };
  const techParams: any = { limit: 10, available: true };
  if (coords) {
    Object.assign(shopParams, { lat: coords.lat, lng: coords.lng, radius: 50 });
    Object.assign(techParams, { lat: coords.lat, lng: coords.lng, radius: 50 });
  }
  if (activeCategory !== "all") shopParams.category = activeCategory;

  const { data: shopsData, isLoading: shopsLoading, refetch: refetchShops } =
    useListShops({ query: { params: shopParams } });
  const { data: techsData, isLoading: techsLoading, refetch: refetchTechs } =
    useListTechnicians({ query: { params: techParams } });

  const shops = (shopsData as any)?.shops ?? [];
  const technicians = (techsData as any)?.technicians ?? [];
  const isLoading = (shopsLoading || techsLoading) && !coords;
...
function ShopOwnerDashboard() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: 16, paddingTop: topPad + 16 }}>
      ...
      <View style={{ marginTop: 24 }}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Quick Actions</Text>
        <DashboardLink icon="package"     label="Manage Products" onPress={() => router.push("/shop/products")} colors={colors} />
